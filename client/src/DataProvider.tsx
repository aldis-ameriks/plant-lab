...
const DataProvider: React.FC<Props> = ({ date = subDays(new Date(), 90).toISOString(), nodeId = '4', render }) => (
  <Query pollInterval={30000} variables={{ date, nodeId }} query={query}>
    {({ loading, error, data }: any) => {
      ...
      const emaValues = ema(temperatureValues, temperatureValues.length / 2).map((value: any) => Math.round(value));
